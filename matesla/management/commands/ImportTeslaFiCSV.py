    ...
    def handle(self, *args, **options):
        ...
        with path.open(newline="", encoding="utf-8-sig") as f:
            ...
            if not reader.fieldnames or "Date" not in reader.fieldnames:
                raise CommandError(
                    "CSV missing Date column (not a TeslaFi monthly export?)"
                )
            ...
            for i, row in enumerate(reader, start=1):
                try:
                    ...
                    # Per-row savepoint: one IntegrityError must not abort the rest
                    with transaction.atomic():
                        ...
                        if existing:
                            self._apply_row_onto(existing, vin, row, utc_dt)
                            # Keep existing.Date if changing it would violate unique
                            # (vin, Date); only move when free or same second.
                            if existing.Date != utc_dt:
                                clash = (
                                    TeslaCarDataSnapshot.objects.filter(
                                        vin=vin, Date=utc_dt
                                    )
                            ...
                            try:
                                ...
                                # Race / exact Date collision: merge into the row
                                existing = (
                                    TeslaCarDataSnapshot.objects.filter(
                                        vin=vin, Date=utc_dt
                                    ).first()
                                    or TeslaCarDataSnapshot.objects.filter(
                                        vin=vin,
                                        Date__gte=minute,
                                        Date__lt=minute_end,
                                    )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                ...
                f"errors={errors} vin={sample_vin} tz={options['tz']}→UTC"
            )
        )

    @staticmethod
    def _apply_row_onto(existing, vin, row, utc_dt):
        """Copy TeslaFi flat fields onto an existing snapshot (keep identity)."""
        existing.merge_from_flat_row(row)
        snap = TeslaCarDataSnapshot()
        snap.apply_flat_row(vin, row, utc_dt)
